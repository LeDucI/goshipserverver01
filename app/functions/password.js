...
exports.changePassword = (email, password, newPassword) =>
	...
	new Promise((resolve, reject)=>{
		...
		user.find({email: email})
			...
			.then(user => resolve({status: 200, message: 'Password Updated Successfully!'}))
		...
		user.find({mEmail: email})
			.then(users => {
				...
				if(users.length == 0){
					...
					user.mTemp_password = hash;
					user.mTemp_password_time = new Date();
	...
	new Promise((resolve, reject) => {
		...
		user.find({mEmail: email})
			.then(users => {
				...
				const diff = new Date() - new Date(user.mTemp_password_time);
			...
			.then(user => {
				if(bcrypt.compareSync(token, user.mTemp_password)){
					const salt = bcrypt.genSaltSync(10);
					const hash = bcrypt.hashSync(password, salt);
					user.mHashed_password = hash;
					user.mTemp_password = undefined;
					user.mTemp_password_time = undefined;
